...
async def validation_exception_handler(request: Request, exc: RequestValidationError):
    ...
    try:
        ...
        logger.warning(f"Request body in validation error: {body[:200] if body else 'Empty'}")
...
async def general_exception_handler(request: Request, exc: Exception):
    ...
    if "JSON decode error" in str(exc) or "Expecting value" in str(exc):
        logger.error("JSON decode error detected - request body may be malformed or empty")
        try:
            body = await request.body()
            logger.error(f"Request body that caused JSON error: {body[:500] if body else 'Empty'}")
        except Exception as e:
